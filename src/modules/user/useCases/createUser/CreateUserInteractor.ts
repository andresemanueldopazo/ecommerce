...
export class CreateUserInteractor
  implements Interactor<CreateUserDTO, Error | void> {
  constructor(private readonly userRepo: IUserRepo) {}

  async execute(request: CreateUserDTO): Promise<Error | void> {
    const userEmailOrError = UserEmail.create(request.email);
    if (userEmailOrError instanceof DomainError) return userEmailOrError;
    ...
    if (userOrError instanceof DomainError) return userOrError;
